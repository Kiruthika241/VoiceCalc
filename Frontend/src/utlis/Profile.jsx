...
import { getCurrentUser, saveAuth, clearAuth } from "./auth"; // ✅ make sure this file exists
...
export default function Profile() {
  ...
  const handleSave = () => {
    // ✅ Update localStorage user so Navbar + others see new name
    ...
    <main className="relative w-full bg-gradient-to-b from-white via-pink-50 to-purple-100 text-gray-900 overflow-x-hidden pt-[90px] sm:pt-[110px] min-h-screen scroll-smooth">
      ...
      <section className="max-w-4xl mx-auto bg-white/80 backdrop-blur-md border border-gray-200 rounded-2xl shadow-xl p-6 sm:p-10 mb-16 animate-fade-up">
        ...
        <ul className="text-gray-700 text-sm space-y-3">
          <li>
            🎧 Updated voice tone to <b>Bright</b>
          </li>
          <li>
            🌐 Changed language to <b>{profile.language}</b>
          </li>
          <li>
            🎨 Set theme to <b>{profile.theme}</b>
          </li>
          <li>🔐 Logged in from a new device (Chrome, Windows)</li>
        </ul>
      </section>
